...
class SequentialModelBasedOptimization(object):
    ...
    def select_configuration(self, capital_theta: np.array) -> np.array:
        ...
        return capital_theta[max_index]
    ...
    def expected_improvement(model: sklearn.gaussian_process.GaussianProcessRegressor,
        ...
        # Mean and standard deviation per configuration
        mu, sigma = model.predict(theta, return_std = True)
        z = (mu - f_star) / sigma
        ei_configuration = (mu - f_star) * norm.cdf(z) + sigma * norm.pdf(z)
        return ei_configuration
        #raise NotImplementedError()
    ...
    def update_runs(self, run: typing.Tuple[np.array, float]):
        ...
        if run[1] > self.theta_inc_performance:
            self.theta_inc_performance = run[1] #update of theta performance
            self.theta_inc = run[0] #update of theta_inc
        #raise NotImplementedError()
